...
export default (
  dc: any,
  { defaultModel, typeTabs, selectorTab, editor, ...config }: any
) => {
    ...
    model: {
      ...
      __initTab() {
        ...
        if (content && getPrevCntId(content)) {
          const id = content.getId(); //@ts-ignore
          const tabId = this.getId();
          content.addAttributes({ id, "aria-labelledby": tabId, hidden: true });
          content.removeAttributes(prevSelectorTab); //@ts-ignore
        ...
        if (!content) {
          ...
          const id = content.getId(); //@ts-ignore
          const tabId = this.getId();
          content.addAttributes({ id, "aria-labelledby": tabId, hidden: true }); //@ts-ignore
          this.addAttributes({ [selectorTab]: id, id: tabId }); //@ts-ignore
      ...
      getTabContent() {
        //@ts-ignore
        const id = this.getAttributes()[selectorTab]; //@ts-ignore
        const tabs = this.getTabsType();
        if (!tabs || !id) return; //@ts-ignore
        const contents = tabs.findContents(); //@ts-ignore
