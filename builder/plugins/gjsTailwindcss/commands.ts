...
const getAllComponents = (model: any, result = []) => {
  //@ts-ignore
  result.push(model); //@ts-ignore
  model.components().each((mod) => getAllComponents(mod, result));
  return result;
};

const updateThemeColor = (edito: Editor, color: any) => {
  //@ts-ignore
  ...
  componentsAll.forEach((c) => {
    ...
    if (
      ...
      el.className?.baseVal.match(colorRegex)
    ) {
      ...
      el.className.match(colorRegex)
    ) {
      el.className = el.className.replace(colorRegex, `$1-${color}-$3`); //@ts-ignore
      c.replaceWith(el.outerHTML);
    }
  });
};
...
export default (editor: Editor, opts = {}) => {
  ...
  cm.add("open-update-theme", {
    run(_, sender) {
      ...
      const md = editor.Modal; //@ts-ignore
  ...
  cm.add("get-tailwindCss", {
    run(editor, sender, options = {}) {
      sender?.set && sender.set("active", 0); //@ts-ignore
      const { callback = (twcss) => console.log(twcss) } = options; //@ts-ignore
