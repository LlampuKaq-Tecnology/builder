...
export default (editor: Editor, opts: any = {}) => {
  ...
  const traits = keys(compProps).map((name) => ({
    changeProp: 1,
    type: "checkbox",
      ...
      defaults: {
        ...
        script() {
          ...
          function resize() {
            ...
            if (
              containerStyles.getPropertyValue("box-sizing") !== "border-box"
            ) {
              ...
              //@ts-ignore
              paddingBottom = parseInt(paddingBottom); //@ts-ignore
              paddingLeft = parseInt(paddingLeft); //@ts-ignore
              paddingRight = parseInt(paddingRight); //@ts-ignore

              minW += paddingLeft + paddingRight; //@ts-ignore
            ...
            if (widthRatio > heightRatio) {
              var new_width = minW; //@ts-ignore
              var new_height = Math.ceil(new_width / videoRatio);
            } else {
              var new_height = minH; //@ts-ignore
